...
//@Controller //완전히 다른방식으로 동작함!!!!
@RestController // Controller + ResponseBody
@RequestMapping("/chap06")
public class RestBasicController {

    //@ResponseBody //요청온곳으로 응답함, 자바의 object를 JSON형식으로 자동 변환
    @GetMapping("/basic")
    public String basic(){
        return "어서오세요";
    }
    ...
    //데이터를 받는 방법
    //get 주소를 통해서 받음 -> 쿼리스트링(?) or 쿼리파라미터(url주소방식)
    //post 바디에 데이터를 담아서 보냄 -> 폼형식 or JSON

    //http://172.30.1.1:8181/chap06/getData?id=홍길동&age=20
    @GetMapping("/getData")
    ...
    //http://172.30.1.1:8181/chap06/getData2?num=1&first=이순신
    @GetMapping("/getData2")
    ...
    //http://172.30.1.1:8181/chap06/getData3/홍길동/30
    @CrossOrigin("http://127.0.0.1:5501")
    @GetMapping("/getData3/{name}/{age}")
    public String getData3(@PathVariable String name,
    ...
    //클라이언트에서 json형식으로 데이터를 보낼 수 있음, RequestBody -> JSON을 object로 맵핑
//    @PostMapping("/getJSON")
...
//    }
    @CrossOrigin("http://127.0.0.1:5501")
    @PostMapping("/getJSON")
    public String getJSON(@RequestBody Map<String, String> map) {
    ...
    public ResponseEntity<SimpleVO> 메서드명() {
        ...
        SimpleVO simpleVO = new SimpleVO(1, "hong", "dong", null);
	...
	*/
    ...
    public ResponseEntity<SimpleVO> getData(@RequestParam("num") int num,
        ...
        return new ResponseEntity<>(new SimpleVO(1, "test",  "test", null), HttpStatus.OK);
    }
